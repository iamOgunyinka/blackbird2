...
double Result::getTradeLengthInMinute() const {
  if (entryTime > 0 && exitTime > 0)
    return (exitTime - entryTime) / 60.0;
  return 0;
}
...
void Result::printEntryInfo(std::ostream &logFile) const {
  ...
  logFile << "   Exchange Long:     " << exchNameLong << " (id " << idExchLong
          << ")" << std::endl;
  logFile << "   Exchange Short:    " << exchNameShort << " (id " << idExchShort
          << ")" << std::endl;
  logFile << "   Fees:              " << feesLong * 100.0 << "% / "
          << feesShort * 100.0 << "%" << std::endl;
  logFile << "   Price Long:        " << priceLongIn << " (target)"
          << std::endl;
  logFile << "   Price Short:       " << priceShortIn << " (target)"
          << std::endl;
  logFile << "   Spread:            " << spreadIn * 100.0 << "%" << std::endl;
  logFile << "   Cash used:         " << exposure << " on each exchange"
          << std::endl;
...
void Result::printExitInfo(std::ostream &logFile) const {
  ...
  logFile << "   Duration:          " << getTradeLengthInMinute() << " minutes"
          << std::endl;
  logFile << "   Price Long:        " << priceLongOut << " (target)"
          << std::endl;
  logFile << "   Price Short:       " << priceShortOut << " (target)"
          << std::endl;
  logFile << "   Spread:            " << spreadOut * 100.0 << "%" << std::endl;
  logFile << "   ---------------------------" << std::endl;
  logFile << "   Target Perf Long:  " << targetPerfLong() * 100.0
          << "% (fees incl.)" << std::endl;
  logFile << "   Target Perf Short: " << targetPerfShort() * 100.0
          << "% (fees incl.)" << std::endl;
  logFile << "   ---------------------------\n" << std::endl;
}
...
void Result::reset() {
  ...
  // that will be erased by the very first value entered in the respective
  // arrays. That's why the reset value for min is 1.0 and for max is -1.0.
...
bool Result::loadPartialResult(std::string filename) {
  ...
  if (!resFile || int(resFile.tellg()) == 0)
    return false;

  resFile.seekg(0);
  resFile >> id >> idExchLong >> idExchShort >> exchNameLong >> exchNameShort >>
      exposure >> feesLong >> feesShort >> entryTime >> spreadIn >>
      priceLongIn >> priceShortIn >> leg2TotBalanceBefore >> exitTarget;

  resFile >> maxSpread[idExchLong][idExchShort] >>
      minSpread[idExchLong][idExchShort] >> trailing[idExchLong][idExchShort] >>
...
void Result::savePartialResult(std::string filename) {
  ...
  resFile << maxSpread[idExchLong][idExchShort] << '\n'
          ...
          << trailingWaitCount[idExchLong][idExchShort] << std::endl;
}
